...
const UserProfileView = ({ onClose, onLogout, onVoiceCommand }) => {
  ...
  const handleVoiceCommand = (command, extra) => {
    ...
    } else if (command === "goToMain" || command === "goToProfile" || command === "goToHistory") {
      // Estos comandos se pasan al componente padre para navegación global
  ...
  const getCurrentScreen = () => {
    if (currentSection === 'menu') return 'profile_menu';
    if (currentSection === 'personal') return 'profile_personal';
    if (currentSection === 'payment') return 'profile_payment';
    if (currentSection === 'support') return 'profile_support';
    return 'profile_menu';
  };
        ...
        <View key={key} style={styles.inputContainer}>
          <Text style={styles.inputLabel}>
            {key === 'name' ? 'Nombre Completo' : 
             key === 'phone' ? 'Teléfono' : 
             key === 'emergency' ? 'Contacto de Emergencia' : 'Correo Electrónico'}
          </Text>
          <TextInput
            style={styles.accessibleInput}
            value={value}
            onChangeText={(text) => setUserData({...userData, [key]: text})}
            onFocus={() => speakGuidance(`Campo ${key === 'name' ? 'nombre completo' : key === 'phone' ? 'teléfono' : key === 'emergency' ? 'contacto de emergencia' : 'correo electrónico'}`)}
  ...
  const renderMenu = () => (
    <ScrollView contentContainerStyle={styles.profileScroll}>
      <View style={{ alignItems: 'center', marginBottom: 30 }}>
        ...
        <Text style={styles.profileName} accessibilityRole="header">Juan Pérez</Text>
  ...
  return (
    <View style={styles.fullScreenContainer}>
      ...
      {/* Botón de voz flotante centrado en la parte inferior */}
...
const styles = StyleSheet.create({
  ...
  profileBody: { flex: 1, padding: 20, paddingBottom: 100 },
  ...
  secondaryButton: { backgroundColor: '#000', height: 55, borderRadius: 10, justifyContent: 'center', alignItems: 'center', marginBottom: 25 },
  ...
  // Wrapper para el botón de voz (permite que el VoiceButton mantenga su propio posicionamiento)
  voiceButtonWrapper: {
    position: 'absolute',
    bottom: 0,
    left: 0,
