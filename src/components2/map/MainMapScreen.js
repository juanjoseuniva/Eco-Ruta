...
const MainMapScreen = ({ onNavigateToTrip, destination, setDestination, setDestinationAddress, onLogout, tripHistory, onVoiceCommand }) => {
  ...
  useEffect(() => {
    if (activeTab === 'home') {
      speakGuidance('Pantalla principal. Mapa de navegación. Di buscar destino para empezar, o di el nombre de un lugar');
    }
  }, [activeTab]);
  ...
  const renderContent = () => {
    if (activeTab === 'home') {
      ...
      return <WalletScreen history={tripHistory} onVoiceCommand={handleVoiceCommand} />;
    }
  };
